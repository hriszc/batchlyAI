...
export async function handleDiagEmail(request: Request): Promise<Response> {
  ...
  const limit = checkRateLimit(`diag:email:${session.user.id}`, 2, 60 * 60);
  if (!limit.allowed) {
    return jsonResponse({ error: "Too many diagnostic emails. Please try later." }, 429);
  }
  ...
  try {
    ...
    if (resp.ok) {
      return jsonResponse({ ok: true, method: "mailchannels", email: session.user.email }, 200);
    }
...
export const Route = createFileRoute("/api/diag/email")({
  server: {
    handlers: {
      POST: async ({ request }) => handleDiagEmail(request),
    },
  },
});
