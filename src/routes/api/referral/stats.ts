...
export async function handleGetReferralStats(request: Request): Promise<Response> {
  ...
  try {
    ...
    const [creditSum] = await db
      .select({
        total: sql<number>`COALESCE(SUM(${referral.referrerCreditsAwarded}), 0)`.mapWith(Number),
      })
      .from(referral)
    ...
    const [commissionSum] = await db
      .select({
        total: sql<number>`COALESCE(SUM(${referral.purchaseCommissionAwarded}), 0)`.mapWith(Number),
      })
      .from(referral)
      .where(and(eq(referral.referrerId, userId), sql`${referral.purchaseCommissionAwarded} > 0`));
...
export const Route = createFileRoute("/api/referral/stats")({
  server: {
    handlers: {
      GET: async ({ request }) => handleGetReferralStats(request),
    },
  },
});
