...
import { workLike, work } from "@/lib/db/schema/data-flywheel.schema";
    ...
    handlers: {
      POST: async ({ request }) => {
        ...
        try {
          ...
          const [existing] = await db.select().from(workLike).where(and(eq(workLike.workId, workId), eq(workLike.userId, session.user.id)));

          if (existing) {
            await db.delete(workLike).where(eq(workLike.id, existing.id));
            await db.update(work).set({ likeCount: sql`${work.likeCount} - 1` }).where(eq(work.id, workId));
            return jsonResponse({ liked: false, likeCount: sql`${work.likeCount}` }, 200);
          } else {
            await db.insert(workLike).values({ id: crypto.randomUUID(), workId, userId: session.user.id, createdAt: Math.floor(Date.now() / 1000) });
            await db.update(work).set({ likeCount: sql`${work.likeCount} + 1` }).where(eq(work.id, workId));
            const [updated] = await db.select({ likeCount: work.likeCount }).from(work).where(eq(work.id, workId));
