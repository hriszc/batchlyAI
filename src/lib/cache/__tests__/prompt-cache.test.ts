...
import { getCachedResult, setCachedResult, getExpandCache, setExpandCache } from "@/lib/cache/prompt-cache";

describe("getCachedResult", () => {
  afterEach(() => { delete (globalThis as any).__env__; vi.restoreAllMocks(); });
  ...
  it("returns URLs on cache hit", async () => {
    ...
    (globalThis as any).__env__ = { batchlyai_kv: { get: vi.fn().mockResolvedValue(JSON.stringify(entry)) } };
