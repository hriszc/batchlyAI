...
// Replicate models must always use the async path (createReplicatePredictions + polling).
...
export async function generateImage(params: ImageGenerationParams): Promise<string[]> {
  ...
  if (model === "z-image-fast") {
    throw new Error("Replicate models must use the async path (createReplicatePredictions)");
  }
