...
describe("createPageMeta", () => {
  it("generates OG + Twitter meta tags", () => {
    const meta = createPageMeta({ title: "Test", description: "Desc", path: "/test", locale: "en" });
    expect(meta.meta.find((m: any) => m.property === "og:title")?.content).toBe("Test");
    expect(meta.meta.find((m: any) => m.name === "twitter:card")?.content).toBe("summary_large_image");
  });
  it("uses default ogImage when not provided", () => {
    const meta = createPageMeta({ title: "T", description: "D", path: "/", locale: "en" });
    expect(meta.meta.find((m: any) => m.property === "og:image")?.content).toContain("og-default.png");
  });
  it("sets noindex when requested", () => {
    const meta = createPageMeta({ title: "T", description: "D", path: "/", locale: "en", noIndex: true });
    expect(meta.meta.find((m: any) => m.name === "robots")?.content).toBe("noindex");
  });
  it("includes jsonLd script when provided", () => {
    const meta = createPageMeta({ title: "T", description: "D", path: "/", locale: "en", jsonLd: { "@type": "Article" } });
