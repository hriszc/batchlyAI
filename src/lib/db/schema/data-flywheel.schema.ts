...
export const savedPrompt = sqliteTable(
  ...
  (table) => [index("saved_prompt_user_idx").on(table.userId)],
);
...
export const workLike = sqliteTable(
  ...
  (table) => [uniqueIndex("work_like_unique").on(table.workId, table.userId)],
);
...
export const savedPromptRelations = relations(savedPrompt, ({ one }) => ({
  user: one(user, { fields: [savedPrompt.userId], references: [user.id] }),
}));

export const workRelations = relations(work, ({ one, many }) => ({
  user: one(user, { fields: [work.userId], references: [user.id] }),
...
export const workLikeRelations = relations(workLike, ({ one }) => ({
  work: one(work, { fields: [workLike.workId], references: [work.id] }),
  user: one(user, { fields: [workLike.userId], references: [user.id] }),
}));

export const workCommentRelations = relations(workComment, ({ one }) => ({
  work: one(work, { fields: [workComment.workId], references: [work.id] }),
  user: one(user, {
